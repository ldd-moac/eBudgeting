var MainTblView = Backbone.View.extend({
	...
	initialize: function(options){
		...
		if(options.rootObjectiveId != null) {
			...
			childrenObjectives.url = appUrl('/Objective/' + options.rootObjectiveId + '/childrenWithAllocations/R1');
	...
	processCalculation: function(e) {
		...
		if(answer ==  true) {
			...
			$.ajax({
				url: appUrl('/BudgetProposal/'+ fiscalYear +'/copyFromAllocationToObjectiveAllocation/R1'),
				success: function() {
					// alert
	...
	loadChildren: function(e) {
		...
		if(parentObjective.get('children') == null || parentObjective.get('children').size() == 0 ) {
			var childrenObjectives = new ObjectiveCollection();
			childrenObjectives.url = appUrl('/Objective/' + parentId + '/childrenWithAllocations/R1');
			childrenObjectives.fetch({
				success: _.bind(function() {
					...
					_.each(json, function(objective) {
						...
						_.each(objective.allocationRecordsR1, function(record) {
							objective.sumProposalAmountRequest += record.amountAllocated;
						});
					
						_.each(objective.objectiveAllocationRecordsR1, function(record) {
