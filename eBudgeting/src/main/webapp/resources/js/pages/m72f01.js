var DetailModalView = Backbone.View.extend({
	...
	additionalAllocationTemplate: Handlebars.compile($('#additionalAllocationTemplate').html()),
	...
	events: {
		...
		"click .addtionalAllocationLnk" : "toggleAddtionalAllocation",
		"click .addAdditionalBudgetAllocation": "addAddtionalBudgetAllocation",
	...
	toggleAddtionalAllocation : function(e) {
		var proposalId = $(e.target).parent().attr('data-id');
	...
	addAddtionalBudgetAllocation: function(e) {
		...
		$.ajax({
			type: 'POST',
			url: appUrl('/BudgetProposal/' + currentProposal.get('id') + '/additionalAllocation'),
			data : {
				amount: amount
			}, 
			success: function() {
