...
<div id="headLine">
	<h4>การปรับลดทะเบียนรายการ รอบที่ 2</h4>	
</div>
...
{{#if isStandardItem}}
	...
	<div id="formulaBox">
		...
		<div>
			...
			<div style="height:35px;">
				<input type="text" id="standardPriceTxt" style="width:70px;" value="{{allocationStandardPriceMap.1.standardPrice}}"></input> &times;
			</div>
		</div>
	</div>
	</form>
...
<td><input type="radio" name="rowRdo" id="rdo_{{index}}" value="{{index}}"/></td>
	...
	<td> <a href="#" class="showFormulaToolBar">[{{code}}] {{name}} 
		{{#if standardStrategy}} {{#with  standardStrategy}}
			= {{formatNumber allocationStandardPriceMap.1.standardPrice}} บาท &times; {{{formulaLine formulaColumns false}}}
  		{{/with}} {{/if}} </a>
		{{#if strategies}}
			<div><u>รายการย่อย</u></div>
			<ul>
				{{#each strategies}}
				<li data-id="{{id}}"><a href="#" class="editStrategy">{{name}} = {{formatNumber allocationStandardPriceMap.1.standardPrice}} บาท &times; {{{formulaLine formulaColumns false}}}</a></li>
				{{/each}}
			</ul>
		{{/if}}
	</td>
...
<script src="<c:url value='/resources/js/pages/m64f05.js'/>"></script>
...
$(document).ready(function() {
	...
	if(fiscalYear!= null && fiscalYear.length > 0 ) {
		headLineStr += 	" ปีงบประมาณ " + fiscalYear + "  รอบที่ 2";
