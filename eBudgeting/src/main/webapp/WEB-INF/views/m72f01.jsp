...
<div>
	...
	<table>
		...
		<tr><td>จัดสรรให้หน่วยงานแล้ว</td><td style="text-align:right">{{sumAllocatedRecord proposals}}</td><td> บาท</td></tr>
		<tr><td>จัดสรรเข้ายุทธศาสตร์กลาง</td><td style="text-align:right" id="reservedBudgetCell">{{formatNumber reservedBudget.amountReserved}} </td><td>บาท</td></tr>
		<tr><td>จัดสรรเพิ่มเติมให้หน่วยงาน</td><td style="text-align:right" id="additionalAllocation"></td><td> บาท</td></tr>
...
<div><u>รายการขอตั้งประมาณ</u></div>
	...
	{{#each this}}
		<li data-id="{{id}}">{{owner.abbr}} 
			ขอตั้ง = {{formatNumber amountRequest}} บาท  / 
			จัดสรร = {{#if amountAllocated}} {{formatNumber amountAllocated}} {{else}} 0 {{/if}} บาท /
			<a href="#" class="addtionalAllocationLnk">จัดสรรเพิ่มเติม = {{#if totalAmountAdditions}} {{formatNumber totalAmountAdditions}} {{else}} 0 {{/if}} บาท </a> 
			</li> <div class="additonalAllocationDetail" style="display:none"/>
	{{/each}}
	</ul>
</div>
...
{{#each this}} 
	<li data-id="{{id}}" proposal-id="{{../id}}">
		<b>ขอตั้ง</b>			
		{{name}} : {{{formulaLine this false}}} = {{{formatNumber totalCalculatedAmount}}} บาท / จัดสรร {{{formatNumberNotNull totalCalculatedAllocatedAmount}}} บาท</li>
{{/each}}
</ul>
</script>

<script id="additionalAllocationTemplate" type="text/x-handler-template">
จัดสรรเพิ่มเติม : <input type="text" class="span2 addtionalAllocationInput" proposal-id="{{id}}" /> บาท <button class="btn btn-mini addAdditionalBudgetAllocation"><i class="icon-ok" icon-white"/> ตกลง</button>
<ul>
{{#each additionalAllocations}} 
	<li data-id="{{id}}" proposal-id="{{../id}}">
		<b>จัดสรรเพิ่มวันที่ {{{formatTimeDetail allocationTimeStamp}}}</b> : {{{formatNumber amount}}} บาท</li>
{{/each}}
</ul>
</script>
