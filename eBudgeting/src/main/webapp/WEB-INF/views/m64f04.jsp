...
<div id="headLine">
	<h4>การประมวลผลก่อนการปรับลดงบประมาณครั้งที่ 2</h4> 
</div>
...
<div style="margin-bottom:20px;">
	<button id="processBtn" class="btn btn-primary">ประมวลผลก่อนการปรับลดงบประมาณครั้งที่ 2</button>
</div>
<table class="table table-bordered table-striped">
	<thead>
		<tr>
			<td style="width: 400px;">กิจกรรม</td>
			<td style="width: 100px;">ปรับลดครั้งที่1 ระดับกิจกรรม</td>
			<td style="width: 100px;">ปรับลดครั้งที่1 ระดับรายการ</td>
		</tr>
	</thead>
	<tbody>
	</tbody>
...
<tr data-id={{id}}>
	<td style="padding-left:{{paddingLeft}}px;"> 
		...
		{{/if}}
		{{name}} </td>
	<td style="text-align: right; padding-right: 15px;"> <br/>{{sumAllocationR1 this}}</td>
	<td style="text-align: right; padding-right: 15px;"> <br/>{{sumObjectiveAllocationR1 this}}</td>
</tr>
{{/each}}
</script>

<script src="<c:url value='/resources/js/pages/m64f04.js'/>"></script>

<script type="text/javascript">

Handlebars.registerHelper('sumAllocationR1', function(obj) {
	var sum=0;
	_.forEach(obj.allocationRecordsR1, function(r) {
		sum += r.amountAllocated;
	});
	return addCommas(sum);
});

Handlebars.registerHelper('sumObjectiveAllocationR1', function(obj) {
	var sum=0;
	_.forEach(obj.objectiveAllocationRecordsR1, function(r) {
		sum += r.amountAllocated;
	});
	return addCommas(sum);
});
